<script>
    ...
    //     infoDiv.innerHTML = `
    //         <h3 class="chapter-strapline italic mb-5">${chapter.title}</h3>
    //         <p class="mt-m">${chapter.excerpt}</p>
    //     `;
    // }
    const chapters = [
        {
    ...
    let selectedChapter = $state(chapters[0]);
    ...
    <div class="chapter-container">
        <ul>
            {#each chapters as chapter, index}
                <li>
                    <!--                <Button class="chapter-title" onclick={() => onclick(chapter.number)}>  <h3>Chapter{chapter.number}: {chapter.title}</h3> </Button>-->
                    <Button class="chapter-title {index + 1 === selectedChapter.number ? 'selected-chapter-title' : ''}"
                            onclick={() => selectedChapter = chapters[index]}>
        ...
        <div class="chapter-info">
            <!--            <h3 class="chapter-strapline italic mb-5">Understanding Visas, Residency and Legal Essentials</h3>-->
            <!--            <p class="mt-m">-->
            <!--                This chapter covers everything you need to know about moving to Spain, including visa requirements, finding accommodation, and settling into your new life.-->
            <!--            </p>  -->
            <h3 class="chapter-strapline italic mb-5">{selectedChapter.strapline}</h3>
            <p class="mt-m">
